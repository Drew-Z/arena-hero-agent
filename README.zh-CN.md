...
- 默认编制为 `18 Worker + 7 Vanguard + 7 Ranger = 32`，Core 容量为 160，用于攒够 150 资源兑换奖品并容纳两名单位损失；玩法 v0.14 从第 21 个单位开始提高生产价格。
- Core 主动远离信标，以采集和生存为主，同时让防卫单位分层保护，避免堵塞 Core 路线。
- 将生命周期、威胁等级和单位任务独立分类，覆盖活动警戒、提前撤离、交战、多轴突围和远征队归队。
- 对地图陈旧区域进行侦察，通过真实视线清除已经耗尽的资源记忆，安排返程交付，并在损失后回收掉落资源。
- 舰队损失后按防守阶段重建，并协调同 Tick 防卫火力，减少对残血目标的无效过量攻击。
...
当前验证的协议是 API `v0.1`、玩法规则 `v0.14`、官方 Python SDK `0.2.9`。v0.14 已删除每 Tick 维护费和欠费伤害；第 21 个及后续单位采用动态生产价格。Agent 会使用官方 SDK 预估每次生产价格，并根据服务端结算事件核对实际消耗。版本监控发现确定性漂移时立即停机；存在 24 小时内成功基线时允许连续两次临时拉取失败，第三次失败或没有有效基线时仍按失败关闭处理。
