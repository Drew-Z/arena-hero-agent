...
The supported unattended profile is `--worker-target 18 --beacon-policy retreat`, yielding `18 Workers + 7 Vanguards + 7 Rangers = 32`. The resulting Core capacity is 160, enough to hold the 150-resource redemption target with a two-Unit loss buffer. Production uses the official dynamic-price helper after Unit 20 and stops at the 32-Unit cap.
